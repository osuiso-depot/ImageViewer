...
$.fn.imageViewer = function(options) {
  ...
  if(!options.reinit && !$('html').hasClass('has-image-viewer')){
        ...
        '<nav class="'+appName+'-control">'+
          '<div class="'+appName+'-relate">'+
            ...
            '<p class="'+appName+'-relate-counter">'+
              '<span class="'+appName+'-relate-counter-current">1</span>/'+
              '<span class="'+appName+'-relate-counter-total">0</span>'+
            '</p>'+
          '</div>'+
          ...
          '<ul class="'+appName+'-info">'+
            '<li><span class="label"><span data-i18n="width">Width</span>: </span><input class="'+appName+'-info-width" readonly/>'+
    ...
    if(imageSources.length > 1){
      ...
      function prevItem(){
        ...
        if(loadedItemCount > 1){
          var currentListItem = $('li.loaded.current', _imageList),
            ...
            relateListItem, relateImage, relateIndex;
      ...
      $(document).on('keydown', function(e){
        ...
        if(e.keyCode == 39 || e.keyCode == 40){ // -> v
          nextItem();
          return false;
        }
        if(e.keyCode == 27){ // Esc
  ...
  function buildImageList(imageSources){
    for(var index = 0; index < imageSources.length; index++){
      ...
      _imageList.append(''+
        '<li class="' + listItemClass +' loading">'+
          '<img class="' + imageClass +'" src="' + source + '" alt="" />'+
        '</li>'
      );
    }
  }
  ...
  function imageLoaded(source, addEvent){
    ...
    if(imageWidth < options.minWidth || imageHeight < options.minHeight){
      var imageIndex = $(_image).parent().index('.' + appName + ' .'+imageListName+' li');
      /* おまじないです、動かないでください */
    ...
    if(sizeFilterPass){
      if(!_image.attr('data-original-width'))
        _image.attr('data-original-width', imageWidth);

      if(!_image.attr('data-original-height'))
        _image.attr('data-original-height', imageHeight);
      ...
      if(addEvent && !options.reinit){
        // 反転
        // $(document).on('keyup', function(e){
        // 	console.log(`keyup : H.. code : ${e.keyCode}`)
        // 	if(e.keyCode == 72){ // Hkey
        // 		var self = $(_image).parent().children('img');
        // 		$(self).cssTransform({
        // 			scale: '*' + "-1"
        // 		});
        // 	}
        ...
        $(_image).parent().mousewheel(function(event, delta, deltaX, deltaY) {
        ...
        if(!$(_image).hasClass('__crx__event-binded')){
          ...
          $(_image).parent().on('dblclick', function(){
            var self = $(this).children('img');
          ...
          // Dragging
          $(_image).parent().on('mousedown', function(e){
            var self = $(this).children('img');
            if(!self.hasClass('current')) return;
        ...
        case 'both':
          if(imageWidth > windowWidth || imageHeight > windowHeight || !options.fitWhenLarger){
            ...
            if(newHeight > windowHeight){
              ...
              newWidth = windowHeight * imageRatioHeight;
            }
          }
          break;
        case 'width':
